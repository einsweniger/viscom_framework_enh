...
namespace viscom::enh {
    ...
    GLTexture::GLTexture(unsigned int w, unsigned int h, unsigned int arraySize, const TextureDescriptor& desc) :
        ...
        mipMapLevels(1)
    {
        gl::glBindTexture(gl::GL_TEXTURE_2D_ARRAY, id.textureId);
        gl::glTexStorage3D(gl::GL_TEXTURE_2D_ARRAY, mipMapLevels, descriptor.internalFormat, width, height, depth);
    ...
    GLTexture::GLTexture(unsigned int size, const TextureDescriptor& desc) :
        ...
        mipMapLevels(1)
    {
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexStorage1D(id.textureType, mipMapLevels, descriptor.internalFormat, width);
        gl::glBindTexture(id.textureType, 0);
        InitSampling();
    }
    ...
    {
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexStorage2D(id.textureType, mipMapLevels, descriptor.internalFormat, width, height);
        if (data) {
            gl::glTexSubImage2D(id.textureType, 0, 0, 0, width, height, descriptor.format,
    ...
    */
        ...
        mipMapLevels(numMipLevels)
    {
        ...
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexStorage3D(id.textureType, mipMapLevels, descriptor.internalFormat, width, height, depth);
        if (data) {
            gl::glTexSubImage3D(id.textureType, 0, 0, 0, 0, width, height, depth,
        ...
        mipMapLevels(1)
    {
        ...
        if (id.textureType == gl::GL_TEXTURE_CUBE_MAP) queryType = gl::GL_TEXTURE_CUBE_MAP_POSITIVE_X;
    ...
    {
        gl::glBindImageTexture(imageUnitIndex, id.textureId, mipLevel, gl::GL_TRUE, 0, accessType, descriptor.internalFormat);
    }
    ...
    void GLTexture::AddTextureToArray(const std::string& file, unsigned int slice) const
    {
        ...
        default:
            LOG(FATAL) << "Unknown texture format (" << descriptor.format << ").";
        ...
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexSubImage3D(id.textureType, 0, 0, 0, slice, width, height, 1,
    ...
    {
        glBindTexture(id.textureType, id.textureId);
        switch (id.textureType)
        {
        case gl::GL_TEXTURE_1D:
            gl::glTexSubImage1D(id.textureType, 0, 0, width, descriptor.format, descriptor.type, data);
            break;
        case gl::GL_TEXTURE_2D:
            gl::glTexSubImage2D(id.textureType, 0, 0, 0, width, height, descriptor.format, descriptor.type, data);
            break;
        case gl::GL_TEXTURE_3D:
            gl::glTexSubImage3D(id.textureType, 0, 0, 0, 0, width, height, depth, descriptor.format, descriptor.type, data);
            break;
        default:
    ...
    void GLTexture::DownloadData(std::vector<uint8_t>& data, size_t offset, size_t size) const
    {
        ...
        gl::glBufferData(gl::GL_PIXEL_PACK_BUFFER, width * height * depth * descriptor.bytesPP, nullptr, gl::GL_STREAM_READ);

        gl::glBindTexture(id.textureType, id.textureId);
        gl::glGetTexImage(id.textureType, 0, descriptor.format, descriptor.type, nullptr);
    ...
    {
        auto comp = 0;
        if (descriptor.format == gl::GL_RED) comp = 1;
        else if (descriptor.format == gl::GL_RG) comp = 2;
        else if (descriptor.format == gl::GL_RGB) comp = 3;
        else if (descriptor.format == gl::GL_RGBA) comp = 4;
        else {
        ...
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glGetTexImage(id.textureType, 0, descriptor.format, gl::GL_UNSIGNED_BYTE, nullptr);
    ...
    void GLTexture::UploadData(std::vector<uint8_t>& data) const
    {
        ...
        gl::glBindTexture(id.textureType, id.textureId);
        if (id.textureType == gl::GL_TEXTURE_3D || id.textureType == gl::GL_TEXTURE_2D_ARRAY) {
            glTexSubImage3D(id.textureType, 0, 0, 0, 0, width, height, depth, descriptor.format, descriptor.type, nullptr);
        } else if (id.textureType == gl::GL_TEXTURE_2D || id.textureType == gl::GL_TEXTURE_1D_ARRAY) {
            glTexSubImage2D(id.textureType, 0, 0, 0, width, height, descriptor.format, descriptor.type, nullptr);
        } else {
            glTexSubImage1D(id.textureType, 0, 0, width, descriptor.format, descriptor.type, nullptr);
        }

        glBindTexture(id.textureType, 0);
    ...
    {
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glGenerateMipmap(id.textureType);
        gl::glBindTexture(id.textureType, 0);
    }
    ...
    void GLTexture::ClearTexture(unsigned int mipLevel, const glm::vec4& data) const
    {
        ...
        gl::glClearTexImage(id.textureId, mipLevel, descriptor.format, gl::GL_FLOAT, &data);
    }
    ...
    glm::uvec3 GLTexture::GetLevelDimensions(int level) const
    {
        gl::GLint w, h, d;
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glGetTexLevelParameteriv(id.textureType, level, gl::GL_TEXTURE_WIDTH, &w);
        gl::glGetTexLevelParameteriv(id.textureType, level, gl::GL_TEXTURE_HEIGHT, &h);
        gl::glGetTexLevelParameteriv(id.textureType, level, gl::GL_TEXTURE_DEPTH, &d);
        gl::glBindTexture(id.textureType, 0);
    ...
    {
        if (id.textureType == gl::GL_TEXTURE_2D_MULTISAMPLE) return;
        SetSampleWrap(gl::GL_CLAMP_TO_BORDER);
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexParameterfv(id.textureType, gl::GL_TEXTURE_BORDER_COLOR, glm::value_ptr(color));
        gl::glBindTexture(id.textureType, 0);
    }
    ...
    void GLTexture::SetSampleWrap(gl::GLenum param) const
    {
        if (id.textureType == gl::GL_TEXTURE_2D_MULTISAMPLE) return;
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_WRAP_S, param);
        if (id.textureType == gl::GL_TEXTURE_2D || id.textureType == gl::GL_TEXTURE_3D) {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_WRAP_T, param);
        }
        if (id.textureType == gl::GL_TEXTURE_3D) {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_WRAP_R, param);
        }
        gl::glBindTexture(id.textureType, 0);
    }
    ...
    void GLTexture::SampleLinear() const
    {
        ...
        gl::glBindTexture(id.textureType, id.textureId);
        if (mipMapLevels > 1) {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MAG_FILTER, gl::GL_LINEAR);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MIN_FILTER, gl::GL_LINEAR_MIPMAP_LINEAR);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_BASE_LEVEL, 0);
        } else {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MAG_FILTER, gl::GL_LINEAR);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MIN_FILTER, gl::GL_LINEAR);
        }
        gl::glBindTexture(id.textureType, 0);
    }
    ...
    void GLTexture::SampleNearest() const
    {
        ...
        glBindTexture(id.textureType, id.textureId);
        if (mipMapLevels > 1) {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MAG_FILTER, gl::GL_NEAREST);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MIN_FILTER, gl::GL_NEAREST_MIPMAP_NEAREST);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_BASE_LEVEL, 0);
        } else {
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MAG_FILTER, gl::GL_NEAREST);
            gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_MIN_FILTER, gl::GL_NEAREST);
        }
        gl::glBindTexture(id.textureType, 0);
    }
    ...
    void GLTexture::ActivateShadowMapComparison() const
    {
        if (id.textureType == gl::GL_TEXTURE_2D_MULTISAMPLE) return;
        gl::glBindTexture(id.textureType, id.textureId);
        gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_COMPARE_MODE, gl::GL_COMPARE_REF_TO_TEXTURE);
        gl::glTexParameteri(id.textureType, gl::GL_TEXTURE_COMPARE_FUNC, gl::GL_LESS);
        gl::glBindTexture(id.textureType, 0);
    }
}
